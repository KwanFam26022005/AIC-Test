...
def main():
    ...
    parser.add_argument("--all_videos", action="store_true",
                        help="scan recursively for all videos under the input directory")
    ...
    for fp in pbar:
        ...
        if args.output:
            if len(files) > 1 or os.path.isdir(args.output) or args.output.endswith("/") or args.output.endswith("\\"):
                os.makedirs(args.output, exist_ok=True)
        ...
        parent_dir = os.path.dirname(out_base)
        base_filename = os.path.basename(out_base)
        ...
        pred_txt = os.path.join(pred_dir, base_filename + ".predictions.txt")
        scenes_txt = os.path.join(scenes_dir, base_filename + ".scenes.txt")
        ...
        if args.visualize:
            ...
            vis_img.save(os.path.join(parent_dir, base_filename + ".vis.png"))
